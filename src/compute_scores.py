...
def get_score_one_qa(pred, label, model_name: str) -> float:
    return None
...
def get_score_one(
    ...
    NAME_TO_SCORE_GETTER = {
        # Updated
        "qa":   get_score_one_longbook_qa_eng,
        "sum":  get_score_one_longbook_sum_eng,
        "kv":   get_score_one_kv_retrieval,
        "char": get_score_one_longdialogue_qa_eng,
...
def compute_scores(preds_path, data_name: str, model_name: str):
    ...
    acc, scores = get_score(labels, preds, data_name, model_name)
    print(acc)
    return scores

def evaluate_and_rewrite_jsonl(preds_path, scores):
    # Load the predictions from the JSONL file
    with open(preds_path, 'r', encoding='utf8') as infile:
        data = [json.loads(line) for line in infile]

    # Rewrite the JSONL file with the updated structure
    output_path = preds_path.with_name(f"{preds_path.stem}_evaluated.jsonl")
    with open(output_path, 'w', encoding='utf8') as outfile:
        for item, score in zip(data, scores):
            formatted_score = f"{score:.3f}"
            new_item = {'id': item['id'], 'score': formatted_score}
            new_item.update(item)
            new_item['score'] = formatted_score  # Ensure 'score' is in the correct position
            outfile.write(json.dumps(new_item, ensure_ascii=False) + '\n')


def parse_filename(filename):
    """Extract metadata from the filename pattern: time_task_model_refsN_tempX.jsonl"""
    parts = filename.stem.split('_')
    task = parts[2]  # Extract task name
    model = parts[3]
    num_refs = int(parts[4].replace('refs', ''))
    temp = float(parts[5].replace('temp', ''))
    return model, num_refs, temp, task  # Added task to return values

def compute_scores_with_metadata(preds_path):
    """Compute scores and return with metadata from filename"""
    print("Loading prediction results from", preds_path)
    preds = list(iter_jsonl(preds_path))
    labels = get_labels(preds)
    model, num_refs, temp, task = parse_filename(preds_path)  # Get task from filename
    preds = get_preds(preds, task)  # Use task instead of directory name
    
    acc, _ = get_score(labels, preds, task, model)  # Use extracted task and model
    
    return {
        'model': model,
        'num_refs': num_refs,
        'temperature': temp,
        'task': task,  # Add task to output
        'metric': acc
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--csv_output",     type=str,   default="scores.csv")
    args = parser.parse_args()

    preds_path = Path(args.output_dir) / f"{args.output_name}.jsonl"
    assert preds_path.exists(), f"Predictions not found in: {preds_path}"

    result = compute_scores_with_metadata(preds_path)
    print(f"Task: {result['task']}")
    print(f"Score: {result['metric']:.4f}")
